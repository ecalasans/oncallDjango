...
def patientsOpenOccurrence(request):
    ...
    leito_id = Leito.objects.get(label=dados_recebidos['leito'], setor_id=setor_id, status='O').id
...
def getOccurencies(request):
    ...
    for ano in anos:
        for mes in meses_ano:
            item = Ocorrencia.objects.filter(pac_id=id_paciente, data_add__month=mes, data_add__year=ano).values('id', 'data_add', 'med__first_name', 'med__last_name').order_by('-data_add')
            if not item.exists():
                continue
